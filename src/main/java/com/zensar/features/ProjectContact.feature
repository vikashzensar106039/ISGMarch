Feature: ProjectContact
  ...
  Scenario Outline: Verify user not able to see Edit contact & remove contact button for non ISG Contact  #<DataSetName>
    When user logs in to ISG
    Then Verify for Active Transaction
    Then User clicks on first Active Transaction
    And Click on go to Project contact list
    Then Click on isg contact
    And Verify there is no remove contact button & Edit contact
    Examples: 
      | DataSetName |
      | AddContact  |
